...
def save_output(inputs, preds, save_dir, img_fn, extra_infos=None,  verbose=False, alpha=0.5, org_size = None):
    outs = []
    ...
    # Resizing to org
    bg_pred_resized = F.interpolate(bg_pred, size=(org_size[0], org_size[1]), mode='bilinear')
    bg_pred_resized = cv2.cvtColor(tensor2np(bg_pred_resized)[0], cv2.COLOR_RGB2BGR)
    ...
    for root, dirs, fns in os.walk(img_path):
        ...
        for dir in dirs:
            ...
            for fn in fn_list:
                ...
                J , org_size= preprocess(fn, img_size=crop_size)
                loaders.append(J)
                save_fns.append(fn)
                sizes.append(org_size)
    return loaders,save_fns, sizes
...
def main(args):
    ...
    prediction_dir = os.path.join(args.test_dir,'preds')
    if not os.path.exists(prediction_dir): os.makedirs(prediction_dir)
    
    doc_loader,fns,sizes = test_dataloder(args.test_dir, args.crop_size)
    with torch.no_grad():
